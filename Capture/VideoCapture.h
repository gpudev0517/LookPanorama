...
typedef void (*VideoCaptureCallback)(int camId, unsigned char* data, int len, int bitsperpixel);
...
{
public:
	VideoCapture(VideoCaptureCallback callback, int index, const char* cameraName, float fps);
...
protected:
	...
	void SetSourceFilters(const char* camName);
...
private:
	...
	{
	public:
		CallbackHandler(VideoCaptureCallback cb, int index);
