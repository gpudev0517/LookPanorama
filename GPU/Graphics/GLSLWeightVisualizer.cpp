...
GLSLWeightVisualizer::GLSLWeightVisualizer(QObject *parent) : GPUWeightVisualizer(parent)
{
}

GLSLWeightVisualizer::~GLSLWeightVisualizer()
{
}

void GLSLWeightVisualizer::initialize(int panoWidth, int panoHeight, int viewCount)
{
	...
	ADD_SHADER_FROM_CODE(m_program, "frag", "visualizer");
#else
	m_program->addShaderFromSourceFile(QOpenGLShader::Vertex, "./Shaders/stitcher.vert");
	m_program->addShaderFromSourceFile(QOpenGLShader::Fragment, "./Shaders/visualizer.frag");
#endif
	m_program->link();
	m_program->bind();
...
void GLSLWeightVisualizer::render(WeightMapPaintMode paintMode, GPUResourceHandle fbos[], GPUResourceHandle weights[], int compositeID, int camIndex, int eyeMode)
{
	m_program->bind();
	m_gl->glBindFramebuffer(GL_FRAMEBUFFER, m_fboId);
	...
	m_program->setUniformValue("paintMode", (int)paintMode);
	m_program->setUniformValue("currentIndex", camIndex);
	m_program->setUniformValue("eyeMode", eyeMode);
	m_program->setUniformValue("eyeIndex", compositeID);
...
#if DEBUG_SHADER
	QString strSaveName = QString("weightvisualizer_") + QString::number(compositeID) + ".png";
