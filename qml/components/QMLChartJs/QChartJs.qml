...
Canvas {
	...
	function requestChangeDataPaint(){
		paintType.type = "REPAINT";
        canvas.requestPaint();
	}
    ...
    layer.enabled: true
    onWidthChanged: if(autoRedraw)invalidate()
    onHeightChanged: if(autoRedraw)invalidate()
    ...
    onPaint: {
        ...
        } else if(paintType.type === "REPAINT") {
			chartRenderHandler.bindDataChangeEvent(event.mouseEvent);
            canvas.chartUpdate()
		} else {
