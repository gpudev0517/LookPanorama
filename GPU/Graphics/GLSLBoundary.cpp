...
GLSLBoundary::GLSLBoundary(QObject *parent) : GPUBoundary(parent)
{
}

GLSLBoundary::~GLSLBoundary()
{
}

void GLSLBoundary::initialize(int panoWidth, int panoHeight, int viewCount)
{
	...
	ADD_SHADER_FROM_CODE(m_program, "frag", "boundary");
#else
	m_program->addShaderFromSourceFile(QOpenGLShader::Vertex, "./Shaders/stitcher.vert");
	m_program->addShaderFromSourceFile(QOpenGLShader::Fragment, "./Shaders/boundary.frag");
#endif
	m_program->link();
	m_program->bind();
...
void GLSLBoundary::render(GPUResourceHandle *fbos)
{
	m_program->bind();
	m_gl->glBindFramebuffer(GL_FRAMEBUFFER, m_fboId);
...
#if DEBUG_SHADER
	QString strSaveName = "boundary.png";
